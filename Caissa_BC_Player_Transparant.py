...
import pygame
import Caissa_visualizer as CV
...
def makeMove(currentState, currentRemark, screen, timelimit=10):
    ...
    cv = CV.Caissa_visualizer()
    if currentState.whose_move == WHITE:
        cv.highest = True
    else:
        cv.highest = False

    # Caissa Transparent will always finish her 3 ply
    move, val = minimax_search(currentState, 3, znum, -100000, 100000, screen, cv, move)
    ...
    newRemark = "Prepare to be smote! I've looked " + str(ply) + " moves ahead"
...
def minimax_search(state, ply, znum, alpha, beta, screen, cv, order=None):
    if ply == 0:
    ...
    # updating the visual
    screen.fill((255,255,255))
    cv.show_stats(screen, tracking_dict)
    cv.show_state_graph(screen)
    ...
    for start_coord, possible_move_list in possible_moves.items():
        for piece_move in possible_move_list:
            ...
            if znum is None:
                return (start_coord, piece_move), -200000 * (.5 - state.whose_move), cv
            ...
            if USE_ZOBRIST and zb_table.get(znum, None) != None and zb_table[znum][0] == (ply-1):
                ...
                zobbed = True # this will flag the visualizer later to change the text color
            else:
                tracking_dict['N_STATES_EXPANDED'] += 1
                _, successor_value = minimax_search(new_state, ply-1, znum, alpha, beta, screen, cv, order)
            ...
            if successor_value is None:
                # we flag the cutoffs so we can visually add them later
                if ply == 1:
                    cv.cutoffs[2] = True
                elif ply == 2:
                    cv.cutoffs[1] = True
            ...
            if state.whose_move == WHITE and value < successor_value:
                ...
                if ply == 1:
                    cv.update_ply_1(value, zobbed)
                elif ply == 2:
                    cv.update_ply_2(value, zobbed)
                elif ply == 3:
                    cv.update_best_branch(value)
            ...
            cv.update_working_children(value, zobbed)
...
def introduce():
    ...
    training under Brice (bricecm) and Rene (renec3). I hope you play
    well, mortal.'''
...
def staticEval(state):
    ...
    # values = [0, 0, -1, 1, -3, 3, -4, 4, -5, 5, -3, 3, -100, 100, -2, 2]
    ...
    if IMITATOR_CAPTURES_IMPLEMENTED == False:
        ...
        # values = [0, 0, -1, 1, -3, 3, -4, 4, -1, 1, -3, 3, -100, 100, -2, 2]
    polarity = [-1, 1]
    eval = 0
